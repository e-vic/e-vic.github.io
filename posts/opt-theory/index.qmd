...
abstract: |
  This is a write up of my notes from the Optimisation Theory and Applicaitons course taught by Bahman Gharesifared in 2016 at Queen's University.
---
...
&= 2(||x-x_n||^2 + ||x-x_m||^2) - ||(x-x_n) + (x-x_m)||^2 \ \ (\textnormal{by the parallelogram law}) \\
...
$$2(\underbrace{||x-x_n||^2}{\rightarrow d^2} + \underbrace{||x-x_M||^2}{\rightarrow d^2}) - 4d^2 \rightarrow 0.$$
...
\Rightarrow & ||P_C(y) - P_C(x)||^2 \leq ||y-x|| \ ||P_C(y) - P_C(x)|| \ \ (\textnormal{by Cauchy Schwarz}).
\end{aligned}$$
■
:::
...
This sequence gives  $\{\hat{x}_k\}_{k=1}^\infty$ where 

...
![](figures/Picture39.jpg){width=70% fig-align="center"}

- *Strictly separating* hyperplane for $C_1$ and $C_2$ is $C_1$ is contained in $H^+$ and $C_2$ in $H^-$.

![](figures/Picture40.jpg){width=30% fig-align="center"}
...
![](figures/Picture41.jpg){width=35% fig-align="center"}
...
![](figures/Picture42.jpg){width=30% fig-align="center"}
:::
...
Let $C\in\mathbb{R}^n$ be a non-empty, convex set, and let $\bar{x}\in\bar{C}\setminus \ri(C)$. Then, there exists a supporting hyperplane to $C$ at $\bar{x}$, i.e. there exists a vector $a\in\mathbb{R}^n$ such that 
...
:::
::: {.proof}
We first show that if there is a solution to @eq-inequalitysystem then such scalars cannot exist (contrapositive of $\Leftarrow$). Suppose otherwise, then $\sum_{i=1}^m \lambda_i\ell_i(x) = 0$ and $\sum_{i=1}^m \lambda_i\alpha_i \leq 0$. But since there is a solution to @eq-inequalitysystem, we also have
...
Note that $K$ is an open convex set. Moreover, $K\cap A = \emptyset$, since vectors in $A$ have at least one entry for which $\ell_i(x) - \alpha_i \geq 0$ since @eq-inequalitysystem is inconsistent. Since $K\cap A=\emptyset$, by @prp-separatinghyperplane there exists a hyperplane $H_{(\lambda,\gamma)}$ that separates $K$ and $A$ such that $A\subseteq H$. In particular, 
...
::: {#def-conjugates}
Let $C$ be a convex non-empty set in $\mathbb{R}^n$ and let $f$ be a function on $C$. The *conjugate set* $C^*$of $C$ on $\mathbb{R}^n$ is defined as
...
![](figures/Picture36.jpg){width=40% fig-align="center"}
...
We also note that the set conjugate set $C^*$ and the conjugate function $f^*$ are both convex. Moreover, $\epi(f^*)$ is a closed convex set.
...
::::
:::: {.example-container}
1. Consider $f:\mathbb{R}\rightarrow\mathbb{R}$ such that $x\mapsto f(x) = ax + b$, $a,b\in\mathbb{R}$. 
...
2. Consider $f:\mathbb{R}\rightarrow\mathbb{R}$ such that $x\mapsto (e^x)$. Then
...
::: {#thm-fenchelduality}
# Fenchel Duality
Let $f$ be a convex function over $C$, $g$ be a concave function of $D$. Both $C$ and $D$ are convex sets in $X = \mathbb{R}^n$. Suppose $C\cap D$ has a non-empty relative interior and $\epi_f(C)$ and $\epi_g(D)$ have non-empty interiors. Consider
...
$$\mu = \inf_{x\in C\cap D} (f(x) - g(x)) \geq \sup_{x^*\in C^*\cap D^*}(g^*(x^*)-f^*(x^*)).$$ {#eq-dualityproof1} 
...
It is enough to show that there exists some point $x_0^* \in C^*\cap D^*$ for with the inequality in @eq-dualityproof1 is achieved.

![](figures/Picture37.jpg){width=60% fig-align="center"}

We want to separate the epigrpahs. The natural candidates for separation are 

$$\begin{aligned}
\epi_{f-\mu}(C) = \{(x,\alpha) \mid x\in C, f(x) - \mu\leq \alpha\}, \\
\epi_{g}(D) = \{(x,\alpha) \mid x\in D, g(x) \geq \alpha\},
\end{aligned}$$

noting that for concave functions, the epigraph is written as the infimum. After verifying tha tthe separation theorem can be applied, we get that there exists a hyperplane that separates $\epi_g(D)$ and $\epi_{f-\mu}(C)$, denoted by
...
Now that we have this hyperplane, we note that $H$ is above $\epi_g(D)$ and arbitrarily close to it. I.e.

$$c = \inf_{x\in D} (\langle x,x_0^*\rangle - g(x)) = g^*(x^*),$$ {#eq-dualityproof2}

similarly, $H$ is below $\epi_{f-\mu}(C)$ and arbitrarily close to it

$$c = \sup_{x\in C} (\langle x,x_0^*\rangle - f(x) + \mu) = f^*(x^*) + \mu.$$ {#eq-dualityproof3}

Thus, with @eq-dualityproof2 and @eq-dualityproof3 we have $g^*(x_0^*) = f^*(x_0^*) + \mu$, i.e. the inequality @eq-dualityproof1 is achieved! ■
:::
...
![](figures/Picture38.jpg){width=20% fig-align="center"}
...
Let us take: 
- $D$ to be the positive orthant, $D = \{x \mid x_i\geq 0, i\in\{1,...,n\}\}$
- $C = \{x \mid \sum_{i=1}^n x_i = R\}$
- $f$ to be zero (since we are going to be using Fenchel Duality and need a convex and a concave function)
...
::: {#thm-minimax}
# Minimax 
Let $X$ and $Y$ be two compact convex sets in $\mathbb{R}^n$. Then, 

$$\min_{x\in X} \max_{y\in Y} \langle x,y\rangle = \max_{y\in Y}\min_{x\in X} \langle x,y\rangle.$$
:::
::: {.proof}
Using Fenchel Duality (@thm-fenchelduality) let $C = \mathbb{R}^n$. Define $f(x) = \max_{y\in Y} \langle x,y\rangle$, $x\in C$. By compactness of $Y$, $f$ is continuous and clearly convex on $C$. Consider now the problem

$$\min_{x\in X} \ f(x)$$

which has a solution by the Weierstrass theorem (@thm-weierstrass). Let us now take $g$ to be the zero function on $D=X$. Note that
...
::: {#def-saddlepoint}
# Saddle points
A point $x^*,z^*\in X\times Z$, where $X\subseteq \mathbb{R}^n$ and $Z\subseteq\mathbb{R}^m$, is called a local min-max saddle point of a continuously differentiable function $F$ if there exists open neighbourhoods $U_x^* \subseteq\mathbb{R}^n$ and $U_z^* \subseteq \mathbb{R}^m$ such that
...
::: {#thm-weakduality}
# Weak duality
Let $F:X\times Z\rightarrow\mathbb{R}$ be as before. Then,

$$\sup_{z\in Z}\inf_{x\in X} F(x,z) \leq \inf_{x\in X}\sup_{z\in Z} F(x,z).$$
:::
::: {.proof}
We know $\inf_{x\in X} F(x,z) \leq F(\tilde{x},z)$ for all $z\in Z$, $\tilde{x}\in X$. In particular, we have
...
::: {#thm-primaldual}
Let $F:X\times Z\rightarrow\mathbb{R}$ be as before. Then the following two conditions are equivalent. 

1. $(x^*,z^*)\in X\times Z$ is a saddle point of $F$. (Note: if it isnot specified as local, then saddle points are assumed to be global.)

2. $x^*$ solves the primal problem, $z^*$ solves the dual problem, and the duality gap is zero, i.e.
...
:::
::: {.proof}
Suppose that 1. holds. Then, by definition we have 
...
Then, using Weak duality (@thm-weakduality), we have $\min_{x\in X}\sup_{z\in Z} F(x,z) = \max_{z\in Z}\inf_{x\in X} F(x,z) = F(x^*,z^*)$ and the duality gap is 0.
...
::: {#thm-KKT}
# Karush-Kuhn-Tucker (KKT) necessary conditions of optimality
Suppose that $x^*$ is a solution to the nonlinear programming problem @eq-NLP. Suppose that $X^*$ is regular. Then, there exists unique vectors $\lambda^* = (\lambda_1^*,...,\lambda_m^*)$ and $\mu^* = (\mu_1^*,...,\mu_r^*)$ such that for 
...
:::
::: {.proof}
We have shown this result for the case of equality constraints. Hence, all we need to show here is that $\mu_j^*\geq 0$ for all $j\in A(x^*)$.
...
Suppose now that $\{x^k\}$ is a sequence of minimisers of $\{F^k\}$ over $\mathbb{S}$ (we can say this because of Weierstrass @thm-weierstrass).
...
$$ \inf_{x\in C} \sup_{\mu\geq 0, \lambda\in\mathbb{R}^m} L(x,\lambda, \mu),$$ {#eq-primalstrongduality}

and the dual problem

$$\sup_{\mu\geq 0, \lambda\in\mathbb{R}^n} \inf_{x\in C} L(x,\lambda,\mu).$$ {#eq-dualstrongduality}

::: {#thm-strongduality}
# Strong Duality
Suppose that the convex program @eq-primalstrongduality has a finite solution and that the Slater's condition holds, which is:

*There exists $\bar{x}\in\ri(C)$ such that $g_j(\bar{x}) < 0$ for all $j\in\{1,...,r\}$ and $h_i(\bar{x}) \leq 0$ for all $i\in\{1,...,m\}$.*

Then, there exists $(\lambda^*,\mu^*)$ such that 

$$ P_v = \inf_{x\in C} \ \left( f(x) + \sum_{i=1}^m \lambda^*_i h_i(x) + \sum_{j=1}^r \mu^*_j g_j(x) \right).$$

Moreover, $(\lambda^*,\mu^*)$ is a solution to the dual problem @eq-dualstrongduality and we have $P_v=D_v$. Finally, if $x^*$ is a solution to @eq-primalstrongduality then $(x^*,\lambda^*,\mu^*)$ is a saddle point of the Langrangian, and finally $\mu_j^* g_j(x^*) = 0$ for all $j\in\{1,...,r\}$.
:::


::: {#thm-convextransposition}
# Convex Transposition
Let $C\subseteq\mathbb{R}^n$ be a convex, non-empty set and let 

- $\{g_i\}_{i=0}^r$ a set of onvex functions on $C$
- $\{h_j\}_{j=1}^m$ affine functions on $C$

Suppose that there exists $\tilde{x}\in\ri(C)$ such that $h_j(\tilde{x})\leq 0$, $j=1,...m$. Thenonly one of the following holds

1. There exists $\bar{x}\in C$ such that 

$$\begin{aligned}
& g_i(\bar{x}) < 0, \ i=0,...,r \\
& h_j(\bar{x}) \leq 0, \ j=1,...,m
\end{aligned}$$

2. There exists $\mu,\lambda\geq 0$, $\lambda neq 0$, such that 

$$ \sum_{i=0}^r \lambda_i g_i(x) + \sum_{j=1}^m \mu_j h_j(x) \geq 0, \ x\in C$$
:::
::: {.proof}
This can be proved using separation. Do on your own. 
:::

::: {.proof}
# of Strong Duality Theorem
Let $f^* = P_v$, which is finite by assumption. Consider the system of inequalities

$$ \begin{aligned}
f(x) - f^* <0 \textnormal{ subject to: } & g_i(x) <0, \ i=1,...,r \\
& h_j(x) \leq 0, \ j=1,...,m
\end{aligned}$$

and $x\in C$. Note that $f^* \leq f(x)$ and hence this system of inequalities is inconsisten. Hence, using the previous theorem, there exists $(\lambda_0^*,...,\lambda_r^*)\geq 0$ (not all zero) and $(\mu_1^*,...,\mu_m^*) \geq 0$ such that 

$$ \lambda_0^*(f(x) - f^*) + \sum_{i=1}^r \lambda_i^* g_i(x) + \sum_{j=1}^m \mu_j^* h_j(x) \geq 0.$$

Now, we again claim that $\lambda_0^* \neq 0$. Suppose otherwise, then 

$$ \sum_{i=1}^r \lambda_i^* g_i(x) + \sum_{j=1}^m \mu_j^* h_j(x) \geq 0 \ \forall x\in C.$$

Hence, using the previous theorem (@thm-convextransposition) once more, we conclude that the system 
...
is inconsistent. This contradicts the Slate condition! Thus, $\lambda_0^* \neq 0$ and without loss of generality we say $\lambda_0^* = 1$. 

Thus, $P_v = f^* \leq f(x) + \sum_{i=1}^r \lambda_i^* g_i(x) + \sum_{j=1}^m \mu_j^* h_j(x) = L(x,\lambda^*,\mu^*)$ for $x\in C$. Then,

$$ \begin{aligned}
P_v & \leq \inf_{x\in C} L(x,\lambda^*,\mu^*) \leq \sup_{\lambda, \mu} \inf_{x\in C} L(x,\lambda, \mu) = D_v \\
P_v \leq D_v
\end{aligned}$$

Then, using weak duality (i.e. $P_v\geq D_v$) we conclude that $P_v = D_v$ and the rest of the statement follows. ■
